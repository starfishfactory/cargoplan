...
│   └── 입출항 정보 테이블 (터미널, 선사, 모선명, 항차, 반입마감, 접안/출항 일시, 상태)
│
├── /blog (블로그)
│   ├── 피처드 아티클 (대형 + 소형 3개)
...
graph TD
    ...
    subgraph GNB["GNB 네비게이션"]
        ...
        FARE["/fare<br/>데이터 센터 (해상 운임 추이)"]
        INDICES["/indices<br/>주요 지표"]
...
| `/indices` | 주요 지표 | 컨테이너선 운임지수 조회, PDF 다운로드 | 0.7 | weekly | O |
